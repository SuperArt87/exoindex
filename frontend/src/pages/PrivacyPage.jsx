...
export default function PrivacyPage() {
  ...
  const sectionsRaw = t("privacy.sections", { returnObjects: true })
  const sections = Array.isArray(sectionsRaw) ? sectionsRaw : []

  return (
    <div className="max-w-3xl mx-auto px-4 py-12 w-full">
      <h1 className="text-2xl font-semibold text-slate-100 mb-3">{t("privacy.title")}</h1>

      <div className="rounded-md border border-amber-700/40 bg-amber-950/20 px-4 py-3 mb-8">
        <p className="text-sm text-amber-300/90 leading-relaxed">{t("privacy.disclaimer")}</p>
      </div>
